...
bool push_forward(subforwardlist **sfl, int d) {
    if(*sfl==NULL){
        *sfl = new subforwardlist{d,NULL};
        return(0);
    }
    subforwardlist *new_head = new subforwardlist{d, *sfl};
    *sfl = new_head;
    return(0);
}
...
bool push_back(subforwardlist **sfl, int d){
    subforwardlist** tail_list = sfl;
    if(*sfl == NULL){
       subforwardlist *new_element = new subforwardlist {d, NULL};
        *sfl = new_element;
        return(0);
    }
...
int pop_forward(subforwardlist **sfl){
    ...
    subforwardlist* new_head=(*sfl)->next;
    int temp = (*sfl)->data;
    delete *sfl;
    *sfl = new_head;
    return(temp);
...
int pop_back(subforwardlist **sfl) {
        ...
        if((*sfl)->next == NULL) {
                int temp = (*sfl)->data;
                delete *sfl; //куда указывает последн€€ ссылка
		*sfl = NULL; //перенаправл€ем указатель на NULL;
                return temp;
        }
...
bool push_where(subforwardlist **sfl, unsigned int where, int d) {
        if(where == 0){
                push_forward(sfl, d); return true;
        }
        subforwardlist *prev_pointer = NULL; //указатель на предыдуший элемент недосписка
        subforwardlist *curr_pointer = *sfl; //указатель на текущий элемент недосписка
        for(unsigned int i = 0; i < where; i++) {
                prev_pointer = curr_pointer;
                curr_pointer = curr_pointer->next;
        }
        subforwardlist *new_element = new subforwardlist {d, curr_pointer};
...
unsigned int size(subforwardlist  **sfl){
    int counter = 1;
    subforwardlist* counter_list = *sfl;
    if(*sfl==NULL){
        return(0);
    }
    while(counter_list->next != NULL){
...
void clear(subforwardlist  **sfl) {
        ...
        while(pointer != NULL){
                subforwardlist *temp = pointer->next;
                delete pointer;
                pointer = temp;
        }
        *sfl = pointer;
